    ...
    c(3269, 2836, 2269, 3694, 4432)), url2 = "https://journals.ub.uni-heidelberg.de/cgi-bin/oastats.cgi?repo=ojs;from_date=2015-09-29%2021:39:27;id=jddm:") 
{
    ...
    for (url in urls) {
        html0 = gsub("(\t|\n)", "", paste(readLines(url, encoding = "UTF-8"), 
            collapse = "\n"))
        links = JDDM::matchAll(html0, "https://journals.ub.uni-heidelberg.de/index.php/jddm/article/view/[0-9/]*\\\">PDF")[[1]][, 
            1]
        ...
        years = c(years, rep(gsub("(.*[(]|[)])", "", JDDM::matchOne(html0, 
    ...
    for (i in 1:length(html)) {
        months = paste(paste0(">", c("Jan", "Feb", "Mar", "Apr", 
            ...
            "Dec"), "<"), collapse = "|")
        df = data.frame(c("Intro", JDDM::matchAll(html[i], months)[[1]]), 
            strsplit(as.character(html[i]), months))
        df[, 2] = gsub("<table class=\\\"stats\\\"><tr><th>201[0-9]</th>", 
            "", df[, 2])
        numbers = sapply(JDDM::matchAll(df[, 2], ">[0-9]+<"), 
