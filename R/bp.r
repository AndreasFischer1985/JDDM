...
#' @param breakLabels Logical value indicating if labels of bars should be tranformed by JDDM::labelbreaker. Defaults to F.
    ...
    font.main3 = 4, omitZeros = T, mar = NA, automar = F, ...) 
{
    ...
    if (!is.null(names.arg)) {
        ...
        list(colnames(x)))[[1]]
    if (automar) 
        mar = ifelse(horiz, list(JDDM::mar(labels2 = names.arg)), 
            list(JDDM::mar(labels1 = names.arg)))[[1]]
    if (is.numeric(mar)) {
        mar0 = par("mar")
        par(mar = mar)
    }
    ...
    if (plot != F) {
        ...
        if (breakLabels == T) 
            colnames(b2) = JDDM::labelbreaker(colnames(b2))
        if (is.numeric(breakLabels)) 
            colnames(b2) = JDDM::labelbreaker(colnames(b2), breakLabels[1])
        if (!horiz) {
            if (axes != F) {
